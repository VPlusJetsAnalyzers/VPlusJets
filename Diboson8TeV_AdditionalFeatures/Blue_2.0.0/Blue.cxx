...
void Blue::SolveRelUnc(const Double_t Dx){
  ...
  // Loop until convergence (or at most 20 times)
  Int_t    Iok = 0, ILo = 0;
  ...
  while(Iok == 0){
    ...
    if(ILo == 20 && Iok == 0){
      printf("... Blue->SolveRelUnc(%5.3f%%): No convergence after %2i iterations I give up. \n", Dx, Iok);
      Iok = 1;
      ILo = 0;
    }
  }
...
Double_t Blue::Likelihood(const Double_t *x, const Double_t *par) const {
  ...
  //              xm = x    <==> combined x for xi determining Obs = m
  //------------------------------------------------------------------------------
...
void Blue::CalcChiRes(const Int_t n, const Int_t IsLike, const Double_t xt) const {
  ...
  // Calculate Covariance as function of xt
