...
void B_EPJC_74_3004(Int_t Flag = 0){
  ...
  TString FilNam = &Buffer[0];
  FilNam = "B_EPJC_74_3004" + FilNam;

  TString InsDef = FilNam + "_Defa";
  TString InsRed = FilNam + "_Redc";
  TString InsMax = FilNam + "_Maxv";
  TString TexDef = FilNam + "_Defa_t";
  TString TexRed = FilNam + "_Redc_t";
  TString TexMax = FilNam + "_Maxv_t";
  TString DisDef = FilNam;

  // names of estimates uncertaintries and observables
  TString NamEst[NumEst] = {"     x1", "     x2"};
  TString NamUnc[NumUnc] = {"   Stat", "   Sys1", "   Sys2"};
  ...
  myBlue->LatexResult(TexDef,ForVal,ForVal,ForRho,ForRho,ForRho);
  ...
  myBlue->PrintRho();
  myBlue->PrintResult();
  myBlue->LatexResult(TexRed,ForVal,ForVal,ForRho,ForRho,ForRho);
  ...
  for(Int_t l=0; l<3 ;l++){
    ...
    if(Flag == 0 && l == 0){
      ...
      myBlue->LatexResult(TexMax,ForVal,ForVal,ForRho,ForRho,ForRho);
    }
    myBlue->PrintResult();
  }
