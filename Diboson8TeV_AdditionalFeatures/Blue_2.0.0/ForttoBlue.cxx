...
void ForttoBlue(TString FilNam = "EPJC_72_2046Fort", TString ForVal = "%5.3f", TString ForRho = "%5.3f"){
  ...
  ret = fgets(buf,200,fpinp);
  ret = fgets(buf,200,fpinp);
  Int_t NumEst = 0, NumUnc = 0, NumObs = 0;
  ...
  //
  // Try to see which marticees are identical
  //
  ...
  for(Int_t i = 0; i < NumEst; i++){
    ...
    ind = ind + 1;
    for(Int_t k = 0; k < NumUnc; k++){
      if(i == NumEst-1 &&  k == NumUnc-1)Format = " " + ForVal;
      IRet = sprintf(c,Format,XEst[ind]); ofs<<c;
      ind = ind + 1;
    }
    IRet = sprintf(c,"\n"); ofs<<c;
  ...
  // the alternative
  IRet = sprintf(c,"\t }else if(Flag == 1){\n"); ofs<<c;
  IRet = sprintf(c,"\t\t myBlue->ReleaseInp(); \n"); ofs<<c;
