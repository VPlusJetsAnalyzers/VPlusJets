...
#### Run the Toy Fits
echo "muons, AntiBtag Resolved. Fit the default fit result with the default configuration"
initRand="$(( 2*${Process} + 166302 ))"
echo "initRand=${initRand}"
CstrNoBtag="runToyMCFits.cc+(${StartToy},${EndToy},1,${initRand},\"Fit_DibosonStandard_mu_log.txt\",\"python -i runDiboson8TevFit.py -j 2 -m Diboson8TeVConfig --runPdfGenToySim --genParamFile DibosonStandardFitOutPars_mu.txt --noNull --nosig\",\"topDibosonParameters_mu.txt dibosonDibosonParameters_mu.txt WpJDibosonParametersMSCorr_mu.txt \",\"./\",\"AntiBtagFitMuon_ShapeBased_VariableYield_GenWpJDataDefaultFitDefault_\",true,true)"
