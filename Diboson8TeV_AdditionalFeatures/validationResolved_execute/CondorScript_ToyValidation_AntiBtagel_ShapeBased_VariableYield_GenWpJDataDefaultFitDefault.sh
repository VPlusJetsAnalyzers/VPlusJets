...
#### Run the Toy Fits
echo "electrons, AntiBtag Resolved. Fit the default fit result with the default configuration"
initRand="$(( 2*${Process} + 266302 ))"
echo "initRand=${initRand}"
CstrNoBtag="runToyMCFits.cc+(${StartToy},${EndToy},2,${initRand},\"Fit_DibosonStandard_el_log.txt\",\"python -i runDiboson8TevFit.py --electron -j 2 -m Diboson8TeVConfig --runPdfGenToySim --genParamFile DibosonStandardFitOutPars_el.txt --noNull --nosig\",\"topDibosonParameters_el.txt dibosonDibosonParameters_el.txt WpJDibosonParametersMSCorr_el.txt QCDDibosonParameters_el.txt \",\"./\",\"AntiBtagFitElectron_ShapeBased_VariableYield_GenWpJDataDefaultFitDefault_\",true,true)"
