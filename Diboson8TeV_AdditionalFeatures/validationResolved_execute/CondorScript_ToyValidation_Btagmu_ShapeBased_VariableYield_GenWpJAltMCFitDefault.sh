...
#### Run the Toy Fits
echo "muons, Btag Resolved. Fit the alternate MC shape parameter toys with the default configuration"
initRand="$(( 2*${Process} + 3079582 ))"
echo "initRand=${initRand}"
CstrNoBtag="runToyMCFits.cc+(${StartToy},${EndToy},3,${initRand},\"Fit_DibosonBtag_mu_log.txt\",\"python -i runDiboson8TevFit.py -j 2 -m Diboson8TeVConfig --runPdfGenToySim --useAlternateModelsToGen --genParamFile MCBtagParametersWpJAlt_mu.txt --btag --noNull --nosig\",\"topDibosonBtagParameters_mu.txt dibosonDibosonBtagParameters_mu.txt WpJDibosonBtagParameters_mu.txt WHbbDibosonBtagParameters_mu.txt \",\"./\",\"BtagFitMuon_ShapeBased_VariableYield_GenWpJAltMCFitDefault_\",true,true)"
