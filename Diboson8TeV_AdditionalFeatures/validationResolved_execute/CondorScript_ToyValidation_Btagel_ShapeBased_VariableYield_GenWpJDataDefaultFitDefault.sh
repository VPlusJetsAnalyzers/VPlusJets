...
#### Run the Toy Fits
echo "electrons, Btag Resolved. Fit the default fit result with the default configuration"
initRand="$(( 2*${Process} + 3566302 ))"
echo "initRand=${initRand}"
CstrNoBtag="runToyMCFits.cc+(${StartToy},${EndToy},3,${initRand},\"Fit_DibosonBtag_el_log.txt\",\"python -i runDiboson8TevFit.py -j 2 --electron -m Diboson8TeVConfig --runPdfGenToySim --genParamFile DibosonBtagFitOutPars_el.txt --btag --noNull --nosig\",\"topDibosonBtagParameters_el.txt dibosonDibosonBtagParameters_el.txt WpJDibosonBtagParameters_el.txt WHbbDibosonBtagParameters_el.txt \",\"./\",\"BtagFitElectron_ShapeBased_VariableYield_GenWpJDataDefaultFitDefault_\",true,true)"
