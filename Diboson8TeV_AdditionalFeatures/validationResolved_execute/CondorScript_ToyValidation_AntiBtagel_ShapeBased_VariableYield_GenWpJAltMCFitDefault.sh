...
#### Run the Toy Fits
echo "electrons, AntiBtag Resolved. Fit the alternate MC shape parameter toys with the default configuration"
initRand="$(( 2*${Process} + 279582 ))"
echo "initRand=${initRand}"
CstrNoBtag="runToyMCFits.cc+(${StartToy},${EndToy},2,${initRand},\"Fit_DibosonStandard_el_log.txt\",\"python -i runDiboson8TevFit.py -j 2 --electron -m Diboson8TeVConfig --runPdfGenToySim --useAlternateModelsToGen --genParamFile MCAntiBtagParametersWpJAlt_el.txt --noNull --nosig\",\"topDibosonParameters_el.txt dibosonDibosonParameters_el.txt WpJDibosonParametersMSCorr_el.txt QCDDibosonParameters_el.txt\",\"./\",\"AntiBtagFitElectron_ShapeBased_VariableYield_GenWpJAltMCFitDefault_\",true,true)"
