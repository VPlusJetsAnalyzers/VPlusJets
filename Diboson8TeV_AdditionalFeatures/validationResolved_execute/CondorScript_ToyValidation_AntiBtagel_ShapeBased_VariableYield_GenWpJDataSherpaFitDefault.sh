...
#### Run the Toy Fits
echo "electrons, AntiBtag Resolved. Fit the WpJSherpa sherpa fit result with the default configuration."
initRand="$(( 2*${Process} + 2588921 ))"
echo "initRand=${initRand}"
CstrNoBtag="runToyMCFits.cc+(${StartToy},${EndToy},2,${initRand},\"Fit_DibosonStandard_el_log.txt\",\"python -i runDiboson8TevFit.py -j 2 --electron -m Diboson8TeVConfig --runPdfGenToySim --genParamFile DibosonStandardWpJSherpaasDefFitOutPars_el.txt --noNull --nosig\",\"topDibosonParameters_el.txt dibosonDibosonParameters_el.txt WpJDibosonParametersMSCorr_el.txt QCDDibosonParameters_el.txt \",\"./\",\"AntiBtagFitElectron_ShapeBased_VariableYield_GenWpJDataSherpaFitDefault_\",true,true)"
