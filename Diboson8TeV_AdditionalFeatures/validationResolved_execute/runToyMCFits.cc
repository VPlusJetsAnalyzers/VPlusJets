...
// -----------------------------------------------------------------------------------------------------------------------------------------------------------//
void processLogs(int fitTypeFlag = -1, int NLogs=1000, const char* processlogPrefix="./temp/BtagFitMuon_ShapeBased_VariableYield_GenWpJDataDefaultFitDefault_", const char* outFileName="./test_LogSummary.root")
//// Processes the logs and produces an output file. Log files are of the form processlogPrefix*.log, where *=0-(NLogs-1)
{
  ...
  double fitTotal(-1.0), fitTotalErr(-1.0), dataTotal(-1.0), chi2dof(-1.0), chi2prob(-1.0);
  int Convergence(-1);
  ...
  OutTree->Branch("fitTotal",&fitTotal,"fitTotal/D");
  OutTree->Branch("fitTotalErr",&fitTotalErr,"fitTotalErr/D");
  OutTree->Branch("dataTotal",&dataTotal,"dataTotal/D");
  ...
  for (int lg=0; lg<NLogs; lg++) {
    ...
    fitTotal=fitResult->fitTotal;
    fitTotalErr=fitResult->fitTotalErr;
    dataTotal=fitResult->dataTotal;
    chi2dof=fitResult->chi2dof;
    chi2prob=fitResult->chi2prob;
    Convergence=fitResult->Convergence;
