...
#### Run the Toy Fits
echo "electrons, Btag Resolved. Fit the WpJSherpa sherpa fit result with the default configuration."
initRand="$(( 2*${Process} + 35588921 ))"
echo "initRand=${initRand}"
CstrNoBtag="runToyMCFits.cc+(${StartToy},${EndToy},3,${initRand},\"Fit_DibosonBtag_el_log.txt\",\"python -i runDiboson8TevFit.py -j 2 --electron -m Diboson8TeVConfig --btag --runPdfGenToySim --genParamFile DibosonBtagWpJSherpaasDefFitOutPars_el.txt --noNull --nosig\",\"topDibosonBtagParameters_el.txt dibosonDibosonBtagParameters_el.txt WpJDibosonBtagParameters_el.txt WHbbDibosonBtagParameters_el.txt \",\"./\",\"BtagFitElectron_ShapeBased_VariableYield_GenWpJDataSherpaFitDefault_\",true,true)"
