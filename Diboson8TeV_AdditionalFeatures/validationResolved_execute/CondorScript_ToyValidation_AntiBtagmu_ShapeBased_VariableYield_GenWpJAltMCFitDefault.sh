...
#### Run the Toy Fits
echo "muons, AntiBtag Resolved. Fit the alternate MC shape parameter toys with the default configuration"
initRand="$(( 2*${Process} + 179582 ))"
echo "initRand=${initRand}"
CstrNoBtag="runToyMCFits.cc+(${StartToy},${EndToy},1,${initRand},\"Fit_DibosonStandard_mu_log.txt\",\"python -i runDiboson8TevFit.py -j 2 -m Diboson8TeVConfig --runPdfGenToySim --useAlternateModelsToGen --genParamFile MCAntiBtagParametersWpJAlt_mu.txt --noNull --nosig\",\"topDibosonParameters_mu.txt dibosonDibosonParameters_mu.txt WpJDibosonParametersMSCorr_mu.txt \",\"./\",\"AntiBtagFitMuon_ShapeBased_VariableYield_GenWpJAltMCFitDefault_\",true,true)"
