...
void kanaelec::myana(double myflag, bool isQCD, int runflag)
{
  ...
  if (myflag == 20110000 || myflag == -100){
    ...
    if (myflag == 20110001 || myflag == -101){
    ...
    else if (myflag == 20110002 || myflag == -102){
    ...
    else if (myflag == 20110003 || myflag == -103){
    ...
    else if (myflag == 20110004 || myflag == -104){
      ...
      std::map<int,TString>::const_iterator it = m_samplecodes_.find(myflag);
      if (it == m_samplecodes_.end()) {
	cerr << "sample code " << (long)myflag << " not recognized, please check" << endl;
	exit(-1);
      }

      TString ntupleprefix = it->second;
      TString ntuplein,rdtreeout;

      ntuplein  = inDataDir + "/el_" + ntupleprefix + ".root";
      rdtreeout = outDataDir + "/RD_el_" + ntupleprefix;

      myChain = new TChain("WJet"); 
      InitCounters( ntuplein, h_events, h_events_weighted);
      myChain->Add( ntuplein ); 
      Init(myChain);Loop( h_events, h_events_weighted, myflag, runflag, rdtreeout, isQCD);
    }
  }
}
...
void kanaelec::Loop(TH1F* h_events, TH1F* h_events_weighted, int wda, int runflag, const char *outfilename, bool isQCD)
{
	...
	TBranch *branch_GroomedJet_numberbjets_csvl = newtree->Branch("GroomedJet_numberbjets_csvl",
								      &GroomedJet_numberbjets_csvl,
								      "GroomedJet_numberbjets_csvl/I");

	Int_t   GroomedJet_numberbjets_csvm = 0; 
	TBranch *branch_GroomedJet_numberbjets_csvm = newtree->Branch("GroomedJet_numberbjets_csvm",
								      &GroomedJet_numberbjets_csvm,
								      "GroomedJet_numberbjets_csvm/I");

	Int_t   GroomedJet_numberbjets_ssvhem = 0; 
	TBranch *branch_GroomedJet_numberbjets_ssvhem = newtree->Branch("GroomedJet_numberbjets_ssvhem",
									&GroomedJet_numberbjets_ssvhem,
									"GroomedJet_numberbjets_ssvhem/I");

	Int_t   GroomedJet_numberbjets_csvl_veto = 0; 
	TBranch *branch_GroomedJet_numberbjets_csvl_veto = newtree->Branch("GroomedJet_numberbjets_csvl_veto",
									   &GroomedJet_numberbjets_csvl_veto,
									   "GroomedJet_numberbjets_csvl_veto/I");

	Int_t   GroomedJet_numberbjets_csvm_veto = 0; 
	TBranch *branch_GroomedJet_numberbjets_csvm_veto = newtree->Branch("GroomedJet_numberbjets_csvm_veto",
									   &GroomedJet_numberbjets_csvm_veto,
									   "GroomedJet_numberbjets_csvm_veto/I");

	Int_t   GroomedJet_numberbjets_ssvhem_veto = 0; 
	TBranch *branch_GroomedJet_numberbjets_ssvhem_veto = newtree->Branch("GroomedJet_numberbjets_ssvhem_veto",
									     &GroomedJet_numberbjets_ssvhem_veto,
									     "GroomedJet_numberbjets_ssvhem_veto/I");

	Int_t   GroomedJet_numberjets = 0;
	TBranch *branch_GroomedJet_numberjets = newtree->Branch("GroomedJet_numberjets",
								&GroomedJet_numberjets,
								"GroomedJet_numberjets/I");

	//lepton, MET angular information
	Float_t GroomedJet_CA8_deltaR_lca8jet[6], GroomedJet_CA8_deltaphi_METca8jet[6], GroomedJet_CA8_deltaphi_Vca8jet[6];
	TBranch *branch_GroomedJet_CA8_deltaR_lca8jet = newtree->Branch("GroomedJet_CA8_deltaR_lca8jet",
									&GroomedJet_CA8_deltaR_lca8jet,
									"GroomedJet_CA8_deltaR_lca8jet[6]/F");
	TBranch *branch_GroomedJet_CA8_deltaphi_METca8jet = newtree->Branch("GroomedJet_CA8_deltaphi_METca8jet",
									    &GroomedJet_CA8_deltaphi_METca8jet,
									    "GroomedJet_CA8_deltaphi_METca8jet[6]/F");
	TBranch *branch_GroomedJet_CA8_deltaphi_Vca8jet = newtree->Branch("GroomedJet_CA8_deltaphi_Vca8jet",
									  &GroomedJet_CA8_deltaphi_Vca8jet,
									  "GroomedJet_CA8_deltaphi_Vca8jet[6]/F");
	...
	Float_t mva126el=-999;
        Float_t mvavbfWjetsel=-999;
        Float_t mvavbfTopel=-999;

	TBranch *branch_mvavbfWjetsel    = newtree->Branch("mvavbfWjetsel",    &mvavbfWjetsel,     "mvavbfWjetsel/F");
        TBranch *branch_mvavbfTopel    = newtree->Branch("mvavbfTopel",    &mvavbfTopel,     "mvavbfTopel/F");
...
//	const char* hvbf_inputVars[] = { "hvbf_lvjj_Rapidity", "hvbf_aj_pt", "hvbf_bj_pt", "hvbf_waj_pt", "hvbf_wbj_pt", "hvbf_event_met_pfmet", "hvbf_wjj_ang_hs", "hvbf_wjj_ang_phib", "hvbf_jj_deta", "hvbf_jj_dphi", "hvbf_jj_m" };
     ...
     const char* hvbf_inputVars[] = { "W_electron_charge", "hvbf_aj_pt", "hvbf_bj_pt", "hvbf_wjj_deta", "hvbf_wjj_m", "hvbf_jj_dphi", "hvbf_jj_m", "hvbf_lvjj_ZeppenField" };
	...
	ReadLikelihoodvbf126el mvaReader126el( hvbf_inputVarsMVA );
      ...
      const char* hvbfWjets_inputVars[] = { "W_electron_charge", "hvbf_wjj_deta", "hvbf_wjj_m", "hvbf_jj_dphi", "hvbf_jj_m", "hvbf_lvjj_Rapidity", "hvbf_lvjj_pt", "hvbf_lvjj_ZeppenField" };

        std::vector<std::string> hvbfWjets_inputVarsMVA;
        for (int i=0; i<8; ++i) hvbfWjets_inputVarsMVA.push_back( hvbfWjets_inputVars[i] );
	ReadLikelihoodvbfWJetsel mvaReaderWjetsel( hvbfWjets_inputVarsMVA );
     ...
     const char* hvbfTop_inputVars[] = { "hvbf_wjj_ang_hs", "hvbf_wjj_m", "hvbf_jj_dphi", "hvbf_jj_m", "hvbf_hW_tag1_deta", "hvbf_lW_tag1_deta", "hvbf_lW_hW_deltaphi", "hvbf_lvjj_ZeppenField", "hvbf_jjj_m" };
        ...
        std::vector<std::string> hvbfTop_inputVarsMVA;
           ...
           ReadLikelihoodvbfTopel mvaReaderTopel( hvbfTop_inputVarsMVA );
	...
	std::vector<std::string> inputVarsMVA_b;
	for (int i=0; i<8; ++i) inputVarsMVA_b.push_back( inputVars_b[i] );
	...
	ReadMVA3j170el mvaReader3j170el( inputVarsMVA_b );  
	ReadMVA3j180el mvaReader3j180el( inputVarsMVA_b );  
	ReadMVA3j190el mvaReader3j190el( inputVarsMVA_b );  
	ReadMVA3j200el mvaReader3j200el( inputVarsMVA_b );  
	ReadMVA3j250el mvaReader3j250el( inputVarsMVA_b );  
	ReadMVA3j300el mvaReader3j300el( inputVarsMVA_b );  
	ReadMVA3j350el mvaReader3j350el( inputVarsMVA_b );  
	ReadMVA3j400el mvaReader3j400el( inputVarsMVA_b );  
	ReadMVA3j450el mvaReader3j450el( inputVarsMVA_b );  
	ReadMVA3j500el mvaReader3j500el( inputVarsMVA_b );  
	ReadMVA3j550el mvaReader3j550el( inputVarsMVA_b );  
	ReadMVA3j600el mvaReader3j600el( inputVarsMVA_b );  
  ...
  4.228E-02,
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) {
		...
		for (int i=0; i<6; i++) {
		  GroomedJet_CA8_deltaR_lca8jet[i] = -999; GroomedJet_CA8_deltaphi_METca8jet[i] = -999; GroomedJet_CA8_deltaphi_Vca8jet[i] = -999;
		}
...
*/
		...
		if ( !isQCD ) {
			...
			if ( !(event_met_pfmet>30.)  ) isgendevtnojetevt=0;
			...
			if ( !( (-1.<W_electron_mvaTrigV0) )  ) isgendevtnojetevt=0;
      ...
      //cout<<"no of non bjets  "<<hnbnot<<" 1st  " <<nbindex[0]<<" 2nd  "<<nbindex[1]<<"no of b jets  "<<hnbjet<<" 1st  "<<bindex[0]<<" 2nd  "<<bindex[1]<<endl;
		...
		if (event_met_pfmet >35.)
		{
			...
			if (W_mt >30. ) {
				...
				h_events_weighted -> Fill ( istep, effwt*puwt ); 
				istep++;
				if ( abs(JetPFCor_dphiMET[0]) >  0.8 ) {
					h_events          -> Fill ( istep ); 
					h_events_weighted -> Fill ( istep, effwt*puwt ); 
					istep++;
					if ( (abs(JetPFCor_Eta[0]))<2.4 && JetPFCor_Pt[0] > 30. ) {
						h_events          -> Fill ( istep );
						h_events_weighted -> Fill ( istep, effwt*puwt );
						istep++;
						if ( (abs(JetPFCor_Eta[1]))<2.4 && JetPFCor_Pt[1] > 30. ) {
							h_events          -> Fill ( istep );
							h_events_weighted -> Fill ( istep, effwt*puwt );
							istep++;
							if ( Mass2j_PFCor > 66. &&  Mass2j_PFCor < 98. ) {
								h_events          -> Fill ( istep ); 
								h_events_weighted -> Fill ( istep, effwt*puwt ); 
								istep++;
								if ( W_electron_et>35.
									...
									h_events_weighted -> Fill ( istep, effwt*puwt ); 
									istep++;
							if ((JetPFCor_bDiscriminatorCSV[0]<0.244) && (JetPFCor_bDiscriminatorCSV[1]<0.244))
							{
							h_events          -> Fill ( istep );
							h_events_weighted -> Fill ( istep, effwt*puwt );
							istep++;
		...
		//if(hvbf_event==1){
                                       ...
                                       std::vector<double> hvbf_mvaInputVal;
                                        hvbf_mvaInputVal.push_back( W_electron_charge );
		...
		//}
                                        ...
                                        hvbfWjets_mvaInputVal.push_back( W_electron_charge );
                                        ...
                                        mvavbfWjetsel = (float) mvaReaderWjetsel.GetMvaValue( hvbfWjets_mvaInputVal );
                                     //   cout<< "mvavbfWjetsel   "<<mvavbfWjetsel<<endl;
                                        //Top
                                        ...
                                        mvavbfTopel = (float) mvaReaderTopel.GetMvaValue( hvbfTop_mvaInputVal );

                                      //  cout<<"mvavbfTopel  "<<mvavbfTopel<<endl;
...
*/
		...
		if(isgengdboostedWevt){
			...
			leadingjetca8p4.SetPtEtaPhiE(GroomedJet_CA8_pt[0],
			...
			for(int i = 0; i < numPFCorJets; i++)
			{
				if(JetPFCor_Pt[i] > Jpt)
				{
					...
					double tmpdelatR = leadingjetca8p4.DeltaR(ajp);
					...
					if(JetPFCor_bDiscriminatorCSV[i] > btcsvm &&
					   tmpdelatR > 0.8 &&
					   tmpdeltaRlj > TMath::Pi()/2.0)/* Veto the AK5 jet in the CA8 jet cone and
									    Move to CSVM tagger and leptonic and hadronic
									    top should be in the different hemisphere for ttbar control */
					{
					...
					if(JetPFCor_bDiscriminator[i] > btssv &&
					   tmpdelatR > 0.8 &&
					   tmpdeltaRlj > TMath::Pi()/2.0) /*Veto the AK5 jet in the CA8 jet cone and
									    Move to CSVM tagger and leptonic and hadronic
									    top should be in the different hemisphere for ttbar control */
			...
			if(GroomedJet_CA8_deltaR_lca8jet[0] > TMath::Pi()/ 2.0 &&
			   GroomedJet_CA8_deltaphi_METca8jet[0] > 2.0 &&
			   GroomedJet_CA8_deltaphi_Vca8jet[0]   > 2.0)//Tighter Angular Cuts 
			{
				ggdboostedWevt = 1;
			}
			...
			boostedW_lvj_e      = (mup+b_nvp+leadingjetca8p4).E();
			boostedW_lvj_pt     = (mup+b_nvp+leadingjetca8p4).Pt();
			boostedW_lvj_eta    = (mup+b_nvp+leadingjetca8p4).Eta();
			boostedW_lvj_phi    = (mup+b_nvp+leadingjetca8p4).Phi();
			boostedW_lvj_m      = (mup+b_nvp+leadingjetca8p4).M();
			boostedW_lvj_y      = (mup+b_nvp+leadingjetca8p4).Rapidity();
...
//    cout<<"down  mass  "<<leadingjetca8p4_Down.M()<<" up mass "<<leadingjetca8p4_Up.M()<<"original m "<<GroomedJet_CA8_mass[0]<<endl;
		...
		if(isgendevtnojetevt)//One good muon, no jet requirement
		{
			...
			float firsttagjetpt = 30.;
			float secondtagjetpt = 30.;
			float tagjeteta = 4.7;
			float tagjetmass = 300.;
			...
			if(sortjets.size() > 1 && sortjets[0]->Pt() > firsttagjetpt && sortjets[1]->Pt() > secondtagjetpt && fabs(sortjets[0]->Eta()) < tagjeteta && fabs(sortjets[1]->Eta()) < tagjeteta)
			{
				...
				if(sortjets.size() > 2 && sortjets[2]->Pt() > 20.0)// Make sure we have one real traditional jet
				{
					for(unsigned int i = 2; i < sortjets.size(); i++)
					{
						...
						if(findcentraljet != 1 && tmpp4.Pt() > 20.0 && tmpp4.Eta() > tagjet_1_2_eta[0] && tmpp4.Eta() < tagjet_1_2_eta[1]) //Central jet between the tag jet eta region
						{
							findcentraljet = 1;
		...
		branch_mva126el->Fill();
                branch_mvavbfWjetsel->Fill();
                branch_mvavbfTopel->Fill();
