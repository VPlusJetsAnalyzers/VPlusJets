...
##     tag += 'Btag'
output = TFile("Mjj%slnujj_%s_output.root" % (tag, mode),"recreate")
fr.Write()
## if opts.nullFit:
##     fr_null.Write()
plot1.Write()
...
c1.SaveAs("Mjj%slnujj_%s_Stacked.png" % (tag, mode))
cp1.SaveAs("Mjj%slnujj_%s_Pull.png" % (tag, mode))

if not pars.useTopSideband:
    c2.SaveAs("Mjj%slnujj_%s_Subtracted.png" % (tag, mode))
