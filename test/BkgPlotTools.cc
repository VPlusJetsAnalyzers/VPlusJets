///*** Should be incorporated into PlotTools.cc
#include <iostream>
#include <fstream>
#include <strstream>
#include <vector>
...
////Fill in each histogram in the array h from the files in infilenames.
{
  ...
  TTree* InTree[20];
  for (Int_t n=0; n<nModes; n++) {
    //cout << "filename = " << infilenames[n] << endl;
...
//     fitGaus->SetParNames("Amp","s0","s1");
    ...
    hist->Fit("fitGaus");
    hist->GetFunction("fitGaus")->SetLineColor(1);
    gMinuit->mnmatu(1);
    gStyle->SetOptStat(0);
...
// -----------------------------------------------------------------------------------------------------------------------------------------------------------//
void Plot_CompareShapesAndCuts(const char* TitleName, const char* TreeName, const char* VarName, int NBins, double VarMin, double VarMax, const char* AddRest = 0, const char* inModes = 0, const char* SaveName = 0, bool withErrors = false, bool useDifferingCuts=false, const char* InExtraCuts=0, bool weightTheHistograms=false, const char* weightVar=0, char* XAxisTitle = 0)
...
//// InCuts should be of the form "cut[0] cut[1] cut[2] ..."
{
  ...
  fillHistograms(h,TreeName,VarName,NBins,VarMin,VarMax,AddRest,nModes,infilenames,withErrors,useDifferingCuts,InExtraCuts,weightTheHistograms,weightVar);
...
// -----------------------------------------------------------------------------------------------------------------------------------------------------------//
void Plot_CompareSigVsBkgShapes(const char* TitleName, const char* sigTreeName, const char* sigVarName, const char* bkgTreeName, const char* bkgVarName, int NBins, double VarMin, double VarMax, const char* sigAddRest = 0, const char* bkgAddRest = 0, const char* sigInModes = 0, const char* bkgInModes = 0, double Lumi_pb = 300000., const char* SaveName = 0, bool withErrors = false, char* XAxisTitle = 0)
//// Plots variable VarName for the given modes. The size (area) of each histogram is proportional to the number of given event.  
//// inModes should be of the form "modename[0] infilename[0] evtCount[0] color[0] modename[1] infilename[1] evtCount[1] color[1] modename[2] ... " 
//// evtCount=sigma/nGenerated for the backgrounds before the cuts; nExpected/fb^-1 for the signal after the cuts; modename[0] for the signal = name of the signal distribution.
{
  // int NBins=25;
  vector<TString> sig_modenames, sig_infilenames, bkg_modenames, bkg_infilenames;
  vector<double> sig_evtcounts, bkg_evtcounts;
  vector<int> sig_colors, bkg_colors;
  int sig_nModes = 0;
  int bkg_nModes = 0;
  double maxtemp=0;
  double max=0;
  ...
  readModes(sigInModes,sig_modenames,sig_infilenames,sig_evtcounts,sig_colors,sig_nModes);
  readModes(bkgInModes,bkg_modenames,bkg_infilenames,bkg_evtcounts,bkg_colors,bkg_nModes);

  cout << "Luminosity=" << Lumi_pb/1000. << "fb^{-1}" << endl;
  for (Int_t n=0; n<sig_nModes; n++) {
    sig_evtcounts[n]=sig_evtcounts[n]*Lumi_pb/1000.;
  }
  for (Int_t n=0; n<bkg_nModes; n++) {
    bkg_evtcounts[n]=bkg_evtcounts[n]*Lumi_pb;
  }
  ...
  TLegend *lgnd;
  if ( bkg_nModes<10 ) {
    lgnd = new TLegend(0.7,0.9-0.05*(bkg_nModes+1),0.90,0.9);
  } else {
    lgnd = new TLegend(0.7,0.4,0.90,0.9);
  }
  lgnd->SetFillColor(kWhite);
  ...
  //Define the histograms
  TH1F* h_sig[20];
  fillHistograms(h_sig,sigTreeName,sigVarName,NBins,VarMin,VarMax,sigAddRest,sig_nModes,sig_infilenames,withErrors,false);

  TH1F* h_bkg[20];
  fillHistograms(h_bkg,bkgTreeName,bkgVarName,NBins,VarMin,VarMax,bkgAddRest,bkg_nModes,bkg_infilenames,withErrors);

  TH1F* ha_bkg[20];
  TH1F* ha_sigtotal;

  ///Add the signal modes
  ha_sigtotal=new TH1F(sig_modenames[0],sig_modenames[0],NBins,VarMin,VarMax);
  for (Int_t n=0; n<sig_nModes; n++) {
    entries = h_sig[n]->GetEntries();
    cout << sig_modenames[n] <<": entries=" << entries << endl;
    scale=sig_evtcounts[n]/entries;
    //    evts=evts+evtcounts[n];
    cout << "scale=" << scale << endl;
    h_sig[n]->Scale(scale);
    NSig=NSig+h_sig[n]->Integral();
    cout << h_sig[n]->Integral() << " events expected in signal mode " << n << endl;
    //cout << "integral=" << h[n]->Integral() << endl;
    ha_sigtotal->Add(h_sig[n]);
  }
  ha_sigtotal->SetLineWidth(2);
  ha_sigtotal->SetLineColor(sig_colors[0]);
  lgnd->AddEntry(ha_sigtotal,sig_modenames[0],"l");
  //Make sure the maximum is set to include all events.
  max=ha_sigtotal->GetMaximum();

  ///Stack the background modes
  TString haname;
  char n_char[30];
  for (Int_t n=0; n<bkg_nModes; n++) {
    sprintf(n_char,"%i",n);
    ...
    haname="ha["+haname;
    ha_bkg[n]=new TH1F(haname,haname,NBins,VarMin,VarMax);
    entries=h_bkg[n]->GetEntries();
    // cout << modenames[n] <<": entries=" << entries << endl;
    scale=bkg_evtcounts[n];
    //cout << "scale=" << scale << endl;
    h_bkg[n]->Scale(scale);
    NBkg=NBkg+h_bkg[n]->Integral();
    cout << h_bkg[n]->Integral() << " " << bkg_modenames[n] << " events expected" << endl;
    for (Int_t m=0; m<(n+1); m++) {
      ha_bkg[n]->Add(h_bkg[m]);
    }
    ha_bkg[n]->SetFillColor(bkg_colors[n]);
  }
  maxtemp=ha_bkg[bkg_nModes-1]->GetMaximum();
  if ( maxtemp > max ){
    max=maxtemp;
  }

  cout << "Total Expected Events: " << NSig << " signal vs " << NBkg << " background" << endl;

  ///Draw the stacked backgrounds & the signal
  //  cout << "nModes=" << nMode << endl;
  int k;
  for (Int_t n=0; n<bkg_nModes; n++) {
    k=bkg_nModes-n-1;
    if ( n==0 ) {
      ha_bkg[k]->SetMinimum(-1.0);
      ha_bkg[k]->SetMaximum(max*1.30);
      ha_bkg[k]->Draw();
      // max=ha_bkg[n]->GetMaximum();
    } else {
      ha_bkg[k]->Draw("same");
    }
    lgnd->AddEntry(ha_bkg[k],bkg_modenames[k],"f");
  }
  ha_sigtotal->Draw("same");
...
//   cout <<"entries=" << entries << endl;
  ...
  ha_bkg[bkg_nModes-1]->SetTitle(TitleName);
  ha_bkg[bkg_nModes-1]->SetXTitle(bkgVarName);
  if ( XAxisTitle!=0 ) {
    ha_bkg[bkg_nModes-1]->SetXTitle(XAxisTitle);
  }

  ha_bkg[bkg_nModes-1]->SetYTitle("Expected Event Count");
  //h[0]->SetXTitleOffset(0.2);
  lgnd->Draw();
