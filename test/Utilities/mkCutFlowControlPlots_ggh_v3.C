...
void mkCutFlowControlPlots_ggh_v3(bool domu=false,bool domva=false,
		bool dovbf=false)
{
...
if ( domu )
{	
	additional_cuts.push_back("effwt*puwt*(event_met_pfmet >25. && W_mt >30. && (abs(JetPFCor_dphiMET[0])) >  0.4 && (abs(JetPFCor_Eta[0]))<2.4 && JetPFCor_Pt[0] > 30.  && (abs(JetPFCor_Eta[1]))<2.4 && JetPFCor_Pt[1] > 30. && (Mass2j_PFCor > 66. &&  Mass2j_PFCor < 98.) && W_muon_pt>25. && abs(W_muon_dz000)<0.02 && abs(W_muon_dzPV)<0.5 && abs(W_muon_eta)<2.1 && (JetPFCor_bDiscriminatorCSV[0]<0.244) && (JetPFCor_bDiscriminatorCSV[1]<0.244) && (fit_mlvjj>340. && fit_mlvjj<780.))");

	additional_cuts.push_back("effwt*puwt*( event_met_pfmet >25. && W_mt >30. && (abs(JetPFCor_dphiMET[0]) ) > 0.4 && (abs(JetPFCor_Eta[0]))<2.4 && JetPFCor_Pt[0] > 30.  && (abs(JetPFCor_Eta[1]))< 2.4 && JetPFCor_Pt[1] > 30. && (Mass2j_PFCor > 66. &&  Mass2j_PFCor < 98.) && W_muon_pt>25. && abs(W_muon_dz000)<0.02 && abs(W_muon_dzPV)<0.5 && abs(W_muon_eta)<2.1 && (JetPFCor_bDiscriminatorCSV[0]<0.244) && (JetPFCor_bDiscriminatorCSV[1]<0.244) &&  (fit_mlvjj>340. && fit_mlvjj<780.) && mva2j500mu >0.6)");
}
else
{
        additional_cuts.push_back("effwt*puwt*(event_met_pfmet >35. && W_mt >30. && abs(JetPFCor_dphiMET[0]) >  0.8 && (abs(JetPFCor_Eta[0]))<2.4 && JetPFCor_Pt[0] > 30.  && (abs(JetPFCor_Eta[1]))<2.4 && JetPFCor_Pt[1] > 30. && (Mass2j_PFCor > 66. &&  Mass2j_PFCor < 98.) && W_electron_et>35. &&  abs(W_electron_eta)<2.5 && (JetPFCor_bDiscriminatorCSV[0]<0.244) && (JetPFCor_bDiscriminatorCSV[1]<0.244) && (fit_mlvjj>340. && fit_mlvjj<780.))");

        additional_cuts.push_back("effwt*puwt*(event_met_pfmet >35. && W_mt >30. && abs(JetPFCor_dphiMET[0]) >  0.8 && (abs(JetPFCor_Eta[0]))<2.4 && JetPFCor_Pt[0] > 30.  && (abs(JetPFCor_Eta[1]))<2.4 && JetPFCor_Pt[1] > 30. && (Mass2j_PFCor > 66. &&  Mass2j_PFCor < 98.) && W_electron_et>35. &&  abs(W_electron_eta)<2.5 && (JetPFCor_bDiscriminatorCSV[0]<0.244) && (JetPFCor_bDiscriminatorCSV[1]<0.244) && (fit_mlvjj>340. && fit_mlvjj<780.) && mva2j500el >0.6)");

} 
  std::vector<std::string> additional_cuts_name;
  additional_cuts_name.push_back("340. < m_{lvjj} < 780.");
  additional_cuts_name.push_back("likelihood >0.6 ");
...
*/
	...
	// Get the input trees:
	// Data
	TFile *fin2,*H500_file,*wwShape_file,*zzShape_file,*wzShape_file,*w4jetShape_file,*w3jetShape_file,*w2jetShape_file,*w1jetShape_file, *ttbar_file,*zjets_file,*stops_file,*stopt_file,*stoptW_file;
	if (domu) 
	{
		...
		//if (dovbf)
		//{
			w4jetShape_file = new TFile("InData/RD_mu_W4Jets_CMSSW532_old.root","READ");
		...
		//}
		//else
		//	w4jetShape_file = new TFile("InData/RD_mu_WJets_CMSSW532_pt1_v2.root","READ");
	...
	{ // electrons
		...
		//if (dovbf)
                 w4jetShape_file = new TFile("InData/RD_el_W4Jets_CMSSW532_old.root","READ");
	         H500_file       = new TFile("InData/RD_el_HWWMH500_CMSSW532_private.root", "READ");
                w3jetShape_file = new TFile("InData/RD_el_W3Jets_CMSSW532.root","READ");
                w2jetShape_file = new TFile("InData/RD_el_W2Jets_CMSSW532.root","READ");
                w1jetShape_file = new TFile("InData/RD_el_W1Jets_CMSSW532.root","READ");

		//else
		//	w4jetShape_file = new TFile("InData/RD_el_WpJ_CMSSW532.root", "READ");
	...
	TTree* treew4j    = (TTree*) w4jetShape_file->Get("WJet");
	...
	// Get Signal MC
/*
	  TH1* th1H500 = (TH1D*) H500_file -> Get("h_events_weighted");
	 th1H500->Sumw2();
	  TH1* th1H500_ext  = new TH1D("th1H500_ext","th1H500_ext", step_max+step_extra, 0, step_max+step_extra);
	double weightSF = 1.;
        weightSF = th1H500 -> GetBinContent(step_max_presel+1+1)/((TH1D*) H500_file -> Get("h_events"))->GetBinContent(step_max_presel+1+1);
	//cout<<"test weightSF   "<<weightSF<<endl;
	for ( int iBin = 1; iBin <= step_max_presel+1; iBin++ ) th1H500_ext -> SetBinContent(iBin, weightSF*th1H500->GetBinContent(iBin));
	for ( int iBin = step_max_presel+1+1; iBin <= step_max+1; iBin++ ) th1H500_ext -> SetBinContent(iBin, th1H500->GetBinContent(iBin));
	for ( int iExtraStep = 0; iExtraStep < step_extra; iExtraStep++ ) 
	{
		treeh500->Draw(TString("numPFCorJets")+TString(">>tmpHist"), additional_cuts[iExtraStep].c_str(), "goff");
		th1H500 -> SetBinContent(step_max+1+iExtraStep, tmpHist->Integral());
		tmpHist -> Reset();
	}
       th1H500_ext -> Sumw2();
...
*/
	...
	for ( int iExtraStep = 0; iExtraStep < step_extra; iExtraStep++ ) {
	treeh500->Draw(TString("numPFCorJets")+TString(">>tmpHist"), additional_cuts[iExtraStep].c_str(), "goff");
	th1H500_ext -> SetBinContent(step_max+1+iExtraStep, tmpHist->Integral());
	tmpHist -> Reset();
	}
	th1H500_ext -> Sumw2();
	...
	th1zz_ext -> Sumw2();
	// Get W4Jets
	TH1* th1w4jets  = (TH1D*) w4jetShape_file -> Get("h_events_weighted");
	th1w4jets->Sumw2();
	TH1* th1w4jets_ext  = new TH1D("th1w4jets_ext","th1w4jets_ext", step_max+step_extra, 0, step_max+step_extra);
	weightSF = th1w4jets -> GetBinContent(step_max_presel+1+1)/((TH1D*) w4jetShape_file -> Get("h_events"))->GetBinContent(step_max_presel+1+1);
	for ( int iBin = 1; iBin <= step_max_presel+1; iBin++ ) th1w4jets_ext -> SetBinContent(iBin, weightSF*th1w4jets->GetBinContent(iBin));
	for ( int iBin = step_max_presel+1+1; iBin <= step_max+1; iBin++ ) th1w4jets_ext -> SetBinContent(iBin, th1w4jets->GetBinContent(iBin));
	for ( int iExtraStep = 0; iExtraStep < step_extra; iExtraStep++ ) {
		treew4j->Draw(TString("numPFCorJets")+TString(">>tmpHist"), additional_cuts[iExtraStep].c_str(), "goff");
		th1w4jets_ext -> SetBinContent(step_max+1+iExtraStep, tmpHist->Integral()); 
		tmpHist -> Reset();
	}
	th1w4jets_ext -> Sumw2();
	...
	// Get W3Jets
	TH1* th1w3jets  = (TH1D*) w3jetShape_file -> Get("h_events_weighted");
	th1w3jets->Sumw2();
	TH1* th1w3jets_ext  = new TH1D("th1w3jets_ext","th1w3jets_ext", step_max+step_extra, 0, step_max+step_extra);
	weightSF = th1w3jets -> GetBinContent(step_max_presel+1+1)/((TH1D*) w3jetShape_file -> Get("h_events"))->GetBinContent(step_max_presel+1+1);
	for ( int iBin = 1; iBin <= step_max_presel+1; iBin++ ) th1w3jets_ext -> SetBinContent(iBin, weightSF*th1w3jets->GetBinContent(iBin));
	for ( int iBin = step_max_presel+1+1; iBin <= step_max+1; iBin++ ) th1w3jets_ext -> SetBinContent(iBin, th1w3jets->GetBinContent(iBin));
	for ( int iExtraStep = 0; iExtraStep < step_extra; iExtraStep++ ) {
		treew3j->Draw(TString("numPFCorJets")+TString(">>tmpHist"), additional_cuts[iExtraStep].c_str(), "goff");
		th1w3jets_ext -> SetBinContent(step_max+1+iExtraStep, tmpHist->Integral()); 
		tmpHist -> Reset();
	}
	th1w3jets_ext -> Sumw2();
	...
	// Get W2Jets
	TH1* th1w2jets  = (TH1D*) w2jetShape_file -> Get("h_events_weighted");
	th1w2jets->Sumw2();
	TH1* th1w2jets_ext  = new TH1D("th1w2jets_ext","th1w2jets_ext", step_max+step_extra, 0, step_max+step_extra);
	weightSF = th1w2jets -> GetBinContent(step_max_presel+1+1)/((TH1D*) w2jetShape_file -> Get("h_events"))->GetBinContent(step_max_presel+1+1);
	for ( int iBin = 1; iBin <= step_max_presel+1; iBin++ ) th1w2jets_ext -> SetBinContent(iBin, weightSF*th1w2jets->GetBinContent(iBin));
	for ( int iBin = step_max_presel+1+1; iBin <= step_max+1; iBin++ ) th1w2jets_ext -> SetBinContent(iBin, th1w2jets->GetBinContent(iBin));
	for ( int iExtraStep = 0; iExtraStep < step_extra; iExtraStep++ ) {
		treew2j->Draw(TString("numPFCorJets")+TString(">>tmpHist"), additional_cuts[iExtraStep].c_str(), "goff");
		th1w2jets_ext -> SetBinContent(step_max+1+iExtraStep, tmpHist->Integral()); 
		tmpHist -> Reset();
	}
	th1w2jets_ext -> Sumw2();
	...
	// Get W1Jets
	TH1* th1w1jets  = (TH1D*) w1jetShape_file -> Get("h_events_weighted");
	th1w1jets->Sumw2();
	TH1* th1w1jets_ext  = new TH1D("th1w1jets_ext","th1w1jets_ext", step_max+step_extra, 0, step_max+step_extra);
	weightSF = th1w1jets -> GetBinContent(step_max_presel+1+1)/((TH1D*) w1jetShape_file -> Get("h_events"))->GetBinContent(step_max_presel+1+1);
	for ( int iBin = 1; iBin <= step_max_presel+1; iBin++ ) th1w1jets_ext -> SetBinContent(iBin, weightSF*th1w1jets->GetBinContent(iBin));
	for ( int iBin = step_max_presel+1+1; iBin <= step_max+1; iBin++ ) th1w1jets_ext -> SetBinContent(iBin, th1w1jets->GetBinContent(iBin));
	for ( int iExtraStep = 0; iExtraStep < step_extra; iExtraStep++ ) {
		treew1j->Draw(TString("numPFCorJets")+TString(">>tmpHist"), additional_cuts[iExtraStep].c_str(), "goff");
		th1w1jets_ext -> SetBinContent(step_max+1+iExtraStep, tmpHist->Integral()); 
		tmpHist -> Reset();
	}
	th1w1jets_ext -> Sumw2();
...
*/
      th1H500_ext->Scale(100.*H500_scale);
      th1H500_ext->SetFillColor(kBlue);
      th1H500_ext->SetLineColor(kBlue);
      th1H500_ext->SetLineWidth(2);
	...
	th1w4jets_ext->Scale(W4Jets_scale);
	th1w4jets_ext->SetFillColor(kRed);
	th1w4jets_ext->SetLineColor(kRed);
	th1w4jets_ext->SetLineWidth(0);

        th1w3jets_ext->Scale(W4Jets_scale);
        th1w3jets_ext->SetFillColor(kRed);
        th1w3jets_ext->SetLineColor(kRed);
        th1w3jets_ext->SetLineWidth(0);

        th1w2jets_ext->Scale(W4Jets_scale);
        th1w2jets_ext->SetFillColor(kRed);
        th1w2jets_ext->SetLineColor(kRed);
        th1w2jets_ext->SetLineWidth(0);

        th1w1jets_ext->Scale(W4Jets_scale);
        th1w1jets_ext->SetFillColor(kRed);
        th1w1jets_ext->SetLineColor(kRed);
        th1w1jets_ext->SetLineWidth(0);
	...
	std::cout << "sig   "  << th1H500_ext->Integral() << std::endl;
	std::cout << "w4jets "  << th1w4jets_ext->Integral()  << std::endl;
        std::cout << "w3jets "  << th1w3jets_ext->Integral()  << std::endl;
        std::cout << "w2jets "  << th1w2jets_ext->Integral()  << std::endl;
        std::cout << "w1jets "  << th1w1jets_ext->Integral()  << std::endl;
	...
	std::cout << "sig  "  << th1H500_ext->Print("all")   << std::endl;
	std::cout << " data " << th1data_ext->Print("all") <<std::endl;

	//std::cout << "w4jets "  << th1w4jets_ext->Print("all")   << std::endl;
        //std::cout << "w3jets "  << th1w3jets_ext->Print("all")   << std::endl;
        //std::cout << "w2jets "  << th1w2jets_ext->Print("all")   << std::endl;
        //std::cout << "w1jets "  << th1w1jets_ext->Print("all")   << std::endl;

       // std::cout << "wjets "  << th1wjets_ext->Print("all")   << std::endl;
		
	
	//std::cout << "tt "   << th1Top_ext->Print("all")   << std::endl;
/*        std::cout << "tt "   << th1Tops_ext->Print("all")   << std::endl;
...
*/
	...
	double den_qcd = 
		...
		th1w4jets_ext->Integral()+
                th1w3jets_ext->Integral()+
                th1w2jets_ext->Integral()+
                th1w1jets_ext->Integral()+
		th1ww_ext->Integral()+
		th1wz_ext->Integral()+
		th1zz_ext->Integral()+
	...
	/*
	   ...
	   th1w4jets->Scale (th1data->Integral()/den); std::cout <<"wjets " << th1w4jets->Integral() << std::endl;
	...
	//	std::cout << "den2 " << den2 << std::endl;
        ...
        TH1D *th1wjets_ext = (TH1D*)th1w4jets_ext->Clone("th1wjets_ext");
        th1wjets_ext->Reset();
        th1wjets_ext->Add(th1w4jets_ext,1);
        th1wjets_ext->Add(th1w3jets_ext,1);
        th1wjets_ext->Add(th1w2jets_ext,1);
        th1wjets_ext->Add(th1w1jets_ext,1);

        TH1D *th1Tops_ext = (TH1D*)th1Top_ext->Clone("th1Tops_ext");
        th1Tops_ext->Reset();
        th1Tops_ext->Add(th1Top_ext,1);
	th1Tops_ext->Add(th1stoptw_ext,1);
	th1Tops_ext->Add(th1stopt_ext,1);
	th1Tops_ext->Add(th1stops_ext,1);
	th1Tops_ext->Add(th1stopptw_ext,1);
	th1Tops_ext->Add(th1stoppt_ext,1);
	th1Tops_ext->Add(th1stopps_ext,1);

        TH1D *th1diboson_ext = (TH1D*)th1ww_ext->Clone("th1diboson_ext");
        th1diboson_ext->Reset();
        th1diboson_ext->Add(th1ww_ext,1);
	th1diboson_ext->Add(th1wz_ext,1);
	th1diboson_ext->Add(th1zz_ext,1);

	// Sum all the backgrounds

	TH1D *th1tot = (TH1D*)th1w4jets_ext->Clone("th1tot");
	th1tot->Reset();
	th1tot->Add(th1diboson_ext,1);
	//th1tot->Add(th1qcd_ext,1);
	th1tot->Add(th1Tops_ext,1);
	...
	{
		binErr = sqrt(
				(th1diboson_ext->GetBinError(i))**2 +
				// (th1qcd_ext->GetBinError(i))**2 +
				(th1Tops_ext->GetBinError(i))**2 +
	...
	//hs->Add(th1mh126_ext);
	hs->Add(th1Tops_ext);
	hs->Add(th1diboson_ext);
	hs->Add(th1wjets_ext);
	hs->Add(th1H500_ext);
        //hs->Add(th1zjets_ext);

		std::cout << "wjets "  << th1wjets_ext->Print("all")   << std::endl;
                std::cout << "tops "   << th1Tops_ext->Print("all")   << std::endl;
                std::cout << "diboson "   << th1diboson_ext->Print("all")    << std::endl;
                std::cout << "zjets "    << th1zjets_ext->Print("all") << std::endl;
	// Set up the legend
	...
	Leg->AddEntry(th1H500_ext,  "sig mH500 x 100",  "f");
	Leg->AddEntry(th1wjets_ext,  "W+jets",  "f");
	Leg->AddEntry(th1ww_ext,  "WW/WZ/ZZ ",  "f");
	Leg->AddEntry(th1Tops_ext,  "top",  "f");
