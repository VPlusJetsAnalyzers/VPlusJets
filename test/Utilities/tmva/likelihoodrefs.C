...
void likelihoodrefs( TString fin = "/uscms_data/d3/ajay/test/forPhil/CMSSW_5_3_2_patch4/src/ElectroWeakAnalysis/VPlusJets/test/Utilities/tmva/WWlvjj_TMVAoutput_174feb/m190/TMVA_190_nJ2_mu.root", Bool_t useTMVAStyle = kTRUE )
{
...
void likelihoodrefs( TDirectory *lhdir ) {
   ...
   Int_t height = 380;
     ...
     mdir = lhdir->GetMotherDir();
//	while(mdir)
//	{

    pmdir =mdir->GetMotherDir();

   const TString filename =pmdir->GetName();
   ...
   while ((key = TMVAGlob::NextKey(next,"TH1"))) { // loop over all TH1
      ...
      if (!found) {
         ...
         if (hname.EndsWith("_sig_nice")) {
            ...
            c[ic]->Update();
            // write to file
            TString fname = Form( "plots/%s_%s_refs_c%i",filename.Data(),titName.Data(), ic+1 );
