...
void variables( TString fin = "/uscms_data/d3/ajay/Higgs125/TMVA/CMSSW_5_3_2_patch4/src/Hwwlnujj/WWlvjj_125TMVAoutput/m125/TMVA_125_nJ2_mu.root", TString dirName = "InputVariables_Id", TString title = "TMVA Input Variables",
...
//                Bool_t isRegression = kFALSE, Bool_t useTMVAStyle = kTRUE )
{
   ...
   while ((key = (TKey*)next())) {
      ...
      // save canvas to file
      if (countPad%noPadPerCanv==0) {
         TString fname = Form( "plots/%s_%s_c%i", fin.Data(), outfname.Data(), countCanvas );
         TMVAGlob::plot_logo();
