...
void mvas( TString fin = "/uscms_data/d3/ajay/Higgs125/TMVA/CMSSW_5_3_2_patch4/src/Hwwlnujj/WWlvjj_125TMVAoutput/m125/TMVA_125_nJ2_mu.root", HistType htype = MVAType, Bool_t useTMVAStyle = kTRUE )
//void mvas( TString fin = "TMVA.root", HistType htype = MVAType, Bool_t useTMVAStyle = kTRUE )
{
   ...
   while ((key = (TKey*)next())) {
      ...
      while ((titkey = (TKey*)keyIt())) {
         ...
         if (Save_Images) {
            if      (htype == MVAType)     TMVAGlob::imgconv( c, Form("plots/%s_mva_%s",     fin.Data(), methodTitle.Data()) );
            else if (htype == ProbaType)   TMVAGlob::imgconv( c, Form("plots/proba_%s",     fin.Data(),    methodTitle.Data()) ); 
            else if (htype == CompareType) TMVAGlob::imgconv( c, Form("plots/overtrain_%s",     fin.Data(),  methodTitle.Data()) ); 
            else                           TMVAGlob::imgconv( c, Form("plots/rarity_%s",     fin.Data(),   methodTitle.Data()) ); 
         }
         countCanvas++;
