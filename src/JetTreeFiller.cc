...
void ewk::JetTreeFiller::SetBranches()
{
	...
	//SetBranch( qgl_out, "Jet" + jetType_ + "_QGL");
	...
	if( jetType_ == "PF" || jetType_ == "PFCor" || jetType_ == "PFCorVBFTag") {
		...
		/// pt_D variable for QG likelihood:  pt_D = sqrt(Sum_i{pt^2})/ Sum_ipt), over all PF cands
...
void ewk::JetTreeFiller::fill(const edm::Event& iEvent){
	...
	edm::Handle<pat::JetCollection> pfjets1;

	//if( jetType=="PF" )
	//  mInputJets = iConfig.getParameter<edm::InputTag>("srcPFJets"); 
	iEvent.getByLabel("selectedPatJetsPFlow", pfjets1);
	// iEvent.getByLabel("ak5PFJetsClean,", pfjets1);


	// if(pfjets1->size() < 1) return;

	//edm::Handle<edm::View<reco::Jet> > pfjets1;
	//iEvent.getByLabel( mInputJets, pfjets1 );
	...
	//      std::cout<<"I am here all outside"<<std::endl;
	//        if(pfjet1.isPFJet()) {
	// if(jetType_=="PF") {
	//if(mInputJets.label()=="PF"){
	//    std::cout<<"I am here outside"<<std::endl;
	// for (reco::PFJetCollection::const_iterator jet = pfjets1->begin();  jet != pfjets1->end(); ++jet){
	//	void ewk::JetTreeFiller::fillQGLH(float)
	//      	 {                         
	//int ijet=0;
	for (pat::JetCollection::const_iterator jet = pfjets1->begin();  jet != pfjets1->end(); ++jet)
	{

		int ijet = jet - pfjets1->begin();
		if( !(ijet<  NUM_JET_MAX) ) break;
		//if( !(iJet< NUM_JET_MAX) ) break;
		// if(jetType_=="PF") {
		//	reco::PFJet pfjet1  = static_cast<const reco::PFJet &>(*jet);
		//	if(pfjet1.isPFJet()) {
		//    const reco::PFJet* pfjet1  = static_cast<const reco::PFJet *>(jet);
		//     edm::RefToBase<reco::Jet> jetRef(edm::Ref<reco::PFJetCollection>(pfjets1,ijet));
		edm::RefToBase<pat::Jet> jetRef(edm::Ref<pat::JetCollection>(pfjets1,ijet));
		//	std::cout<<"I am here inside "<<std::endl;
		//     if (QGTagsHandleMLP.isValid()) std::cout << "MLP: " << (*QGTagsHandleMLP)[jetRef] << std::endl;
		if (QGTagsHandleLikelihood.isValid()) 
		{	PFqgLikelihood[ijet]=(*QGTagsHandleLikelihood)[jetRef];
	...
	for (jet = jets->begin();  jet != endpjets;  ++jet, ++iJet) {
		...
		//     if (QGTagsHandleLikelihood.isValid()) std::cout << "Likelihood: " << (*QGTagsHandleLikelihood)[edm::RefToBase<reco::Jet>] << std::endl;
